...
from ufl import div, dx, grad, inner, SpatialCoordinate, MixedElement
...
Enriched_ref = ufl.FiniteElement("CG",msh_ref.ufl_cell(), 1) + ufl.FiniteElement("Bubble", msh_ref.ufl_cell(), 3)
P1_ref = ufl.FiniteElement("CG", msh_ref.ufl_cell(), 1)

MIXED_SPACE = fem.FunctionSpace(msh_ref, MixedElement([ufl.VectorElement(Enriched_ref,dim=2),P1_ref]))

V_ref, _ = MIXED_SPACE.sub(0).collapse()
Q_ref, _ = MIXED_SPACE.sub(1).collapse()

# P2 = ufl.VectorElement("Lagrange", msh_ref.ufl_cell(), 2)
# P1 = ufl.FiniteElement("Lagrange", msh_ref.ufl_cell(), 1)
...
###### SAVE SOLUTIONS #############
with XDMFFile(MPI.COMM_WORLD, "out_mini/velocity_P1mini.xdmf", "w") as ufile_xdmf:
    u_ref.x.scatter_forward()
    ufile_xdmf.write_mesh(msh_ref)
    ufile_xdmf.write_function(u_ref)

with XDMFFile(MPI.COMM_WORLD, "out_mini/pressure_P1mini .xdmf", "w") as pfile_xdmf:
...
for mesh_iterator in range(0,n_refinements) : # mesh refinements
    ...
    Enriched = ufl.FiniteElement("CG",msh.ufl_cell(), 1) + ufl.FiniteElement("Bubble", msh.ufl_cell(), 3)
    P1 = ufl.FiniteElement("CG", msh.ufl_cell(), 1)

    MIXED_SPACE = fem.FunctionSpace(msh, MixedElement([ufl.VectorElement(Enriched,dim=2),P1]))
...
plt.loglog(maxh[:], maxh[:]**2, '--', label= 'order 2')
gmsh.finalize()
plt.xlabel('$h$')
plt.ylabel('Error L2 norm')
plt.grid(True)
plt.suptitle('Stokes Problem, $\mathbb{P}^1$-$\mathbb{MINI}$ element')
plt.legend()
plt.show()
plt.savefig('stokes_TH_convergence_rates.png')
